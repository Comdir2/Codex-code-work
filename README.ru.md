...
Если вам нужны подключение репозитория к workflow, специальные ограничения для реализации или отдельно согласованная разработка, используйте [центральную форму заявки](https://github.com/Comdir2/Codex-engineering-guardrails/issues/new?template=service-request.yml). Форма создаёт публичный GitHub issue, поэтому указывайте только обезличенное описание. Не публикуйте учётные данные, содержимое закрытых репозиториев, проприетарный исходный код, данные клиентов, приватные ссылки или коммерческие секреты.

Открытие формы — только запрос на обсуждение объёма работ. Оно не является заказом, офертой, договором или разрешением начать работу. Варианты сотрудничества и необязательные начальные диапазоны приведены в разделе [«Услуги»](SERVICES.md).
